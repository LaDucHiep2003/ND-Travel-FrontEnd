  ...
  <div class="container mx-auto">
    <div class="py-4">
      <div class="flex justify-between flex-nowrap min-h-8">
        <div class="py-1.5 text-[22px] font-550">Danh sách khách hàng</div>
        <div class="flex-1 flex items-center self-start ml-4 whitespace-nowrap">
          ...
          <div class="ml-4">
            <RouterLink :to="{ name : 'create_customers'}" class="flex justify-center items-center relative text-center rounded-md min-w-9 min-h-9 py-[7px]
    ...
    <!--Giao dien khi co tai khoan-->
    <div v-if="dataUsers" class="relative rounded-md bg-white overflow-clip css-onvepv h-screen" style="box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 3px 1px, rgba(0, 0, 0, 0.15) 0px 1px 2px 0px;
                outline: transparent solid 1px;}">
      <ul v-if="dataUsers" class="flex flex-wrap border-b border-[#e8eaeb]">
        <li v-for="(user, code) in tabLabels" :key="code" class="flex p-0 m-0 relative">
          <button
              @click="handleChangeTab(user.id)"
          ...
          <span
              ...
              currentTab === user.id ? 'text-color-28 active-tab' : 'text-color-26 button-title-table'
            ]"
          >
            {{ user.name }}
          </span>
      ...
      <LocTaiKhoan @search="handleSearch" />

      <TableKhachHang :dataUsers="dataUsers" />
    </div>
    <!--Khi khong co tai khoan nao-->
      ...
      <div class="px-5 pb-16">
        ...
        <div class="flex flex-col items-center gap-4">
          ...
          <div class="flex flex-col items-center">
            <div class="pb-4">
              <div class="pb-1 text-xl font-550 text-center">Cửa hàng hiện tại chưa có tài khoản nào</div>
              <div class="block text-center text-color-26">Cửa hàng của bạn chưa có tài khoản nào</div>
            </div>
            <div class="flex flex-wrap justify-center">
              <RouterLink :to="{ name : 'create_customers'}" class="inline-flex items-center relative justify-center text-center rounded-md min-w-9 min-h-9
                    ...
                    <span class="flex justify-center items-center tracking-normal">
                      ...
                      <span class="text-sm font-550">Thêm tài khoản</span>
                    </span>
              </RouterLink>
            </div>
          </div>
...
import TableKhachHang from "@/pages/admin/KhachHang/TableKhachHang.vue";
import LocTaiKhoan from "@/pages/admin/KhachHang/LocTaiKhoan.vue";
...
export default {
  ...
  computed: {
    tabLabels() {
      const labels = [{ id: 'all', name: 'Tất cả' }];
      this.roleOptions.forEach(role => {
        labels.push({ id: role.id, name: role.name });
      });
      return labels;
    }
  },
