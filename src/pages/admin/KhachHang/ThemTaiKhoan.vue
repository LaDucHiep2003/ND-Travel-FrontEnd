    ...
    <div class="py-4">
      <div class="flex justify-start min-h-8 items-center">
        ...
        <div class="py-1.5 px-[2px] ">
          <h1 class="font-550 text-[22px]">Thêm tài khoản</h1>
        </div>
      </div>
    </div>
              ...
              <div class="-mt-4 -ml-4">
                ...
                <div class="mt-4 ml-5">
                  <div class="flex justify-between items-center gap-5">
                    ...
                    <div class="w-1/2">
                      <div class="break-words flex flex-wrap justify-between items-baseline pb-1 ">
                        <label for="password" class="text-color-24 css-1ypmoi1">Mật khẩu</label>
                      </div>
                      <div class="flex gap-2">
                        <div class="flex items-center gap-2 relative font-450 text-sm w-full">
                          <input id="password" class="resize-none z-30 w-full text-left border min-h-9 py-2 px-3 rounded-md"
                                 :type="showPassword ? 'text' : 'password'" v-model="dataSent.password" placeholder="Nhập mật khẩu" style="box-shadow: none;outline: none;">
                          <div class="bg-white border border-color-29 z-10 absolute inset-0 pointer-events-none rounded-md"></div>
                        </div>
                        <button type="button" @click="showPassword = !showPassword"
                                class="text-sm focus:outline-none w-10">
    ...
    <div class="py-4 border-t border-color-29">
      <div class="flex justify-end gap-4">
        ...
        border border-transparent text-white bg-color-28 hover:opacity-70">
          <span class="font-550">Thêm tài khoản</span>
        </button>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods:{
    async handleSubmit(){
      const result = await createUser(this.dataSent);
      if(result){
    ...
    selectRole(option) {
      ...
      const newRole = { id: option.value };

      // Tránh thêm trùng
      const exists = this.dataSent.roles.some(r => r.id === option.value);
      if (!exists) {
        this.dataSent.roles.push(newRole);
      }

      // Reset checkbox nếu cần
