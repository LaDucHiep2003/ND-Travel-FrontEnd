      ...
      <table class="w-full max-w-full border-collapse">
        ...
        <tr>
          ...
          <th class="bg-[#f4f6f8] py-3.5 px-4 text-left font-550 whitespace-nowrap">
            <span v-if="!hasSelection">Mô tả </span>
          </th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(item,index) in dataRoles" class="bg-white cursor-pointer border-t border-[#e8eaeb] hover:bg-color-30">
          ...
          <td class="left-[88px] sticky py-2 px-4 text-left whitespace-nowrap">
            <RouterLink to="/" class="inline text-color-28">
              <div class="flex">
                <div class="w-fit">#</div>
                <div class="w-fit text-sm font-450">{{ item.code }}</div>
              </div>
            </RouterLink>
          </td>
