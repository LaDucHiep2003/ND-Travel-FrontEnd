    ...
    <div class="py-4">
      <div class="flex justify-start min-h-8 items-center">
        ...
        <div class="py-1.5 px-[2px] ">
          <h1 class="font-550 text-[22px]">Thêm danh mục</h1>
        </div>
      </div>
    </div>
    ...
    <div class="py-4 border-t border-color-29">
      <div class="flex justify-end gap-4">
        ...
        border border-transparent text-white bg-color-28 hover:opacity-70">
          <span class="font-550">Thêm danh mục</span>
        </button>
      </div>
    </div>
  </div>
...
import {createCategory, getCategory} from "@/service/TourCategoryService.js";
export default {
  components:{
    Editor
  },
  ...
  methods:{
    ...
    async handleSubmit(){
      const result = await createCategory(this.dataSent);
    ...
    async loadCategories(){
      ...
      if(result){
        this.optionsCategory = result.map(cate => ({
          label : cate.name,
          value : cate.id
        }))
      }
    ...
    selectCategory(option) {
      ...
      this.dataSent.parent_id = option.value
